...
public class JwtUtil {
    ...
    public String createJwt(Authentication authentication, Boolean rememberMe, Boolean isRefresh) {
        ...
        return createJwt(isRefresh, rememberMe, user.getId(), user.getUsername(), authUserFormModel.getType(), user.getRoleSet(), user.getPermissionSet());
    }
    ...
    public String createJwt(Boolean isRefresh,
        ...
        JwtClaims jwtClaims = new JwtClaims().setAuthType(authType).setRoles(roles).setPermissions(permissions);
...
//            throw new JeeasyException("token 解析失败: 参数不存在");
    ...
    public JwtTokens refreshJwt(String token) {
        ...
        String jwtToken = createJwt(false, false, claims.getId(), username, jwtClaims.getAuthType(),
                jwtClaims.getRoles(), jwtClaims.getPermissions());
        String refreshJwtToken = createJwt(true, false, claims.getId(), username, jwtClaims.getAuthType(),
                jwtClaims.getRoles(), jwtClaims.getPermissions());
