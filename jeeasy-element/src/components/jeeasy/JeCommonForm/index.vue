...
export default {
  ...
  methods: {
    ...
    generateInput (item) {
      ...
      return <div class="oa-form-item">
        <el-input v-model={ this.formModel[item.name] } type={ item.type } disabled={ item.disabled } autosize={ item.autosize } style={ item.style || this.defaultStyle }/>
      { this.generateShowImg(item) }
      { ele }
    </div>
    },
  ...
  render (h) {
    ...
    return (
      ...
      <div class="center">
      <img src={ this.viewImg } class="max-width-100"/>
      </div>
      </el-dialog>
      </div>
  )
