...
//            SecurityUserDetails<?> principal = (SecurityUserDetails<?>) userInfo;
//            SecurityUserDetails<?> userDTO = authService.getAuthUserByUsername(principal.getUsername()).createUserDetails();
//
//            //获取资源，前后端分离，所以过滤页面权限，只保留按钮权限
//            List<String> btnPerms = userDTO.getPermissions().stream()
//                    // 过滤页面权限
////                    .filter(menuRight -> menuRight.getGrades() >= 3)
//                    // 过滤 URL 为空
//                    .filter(menuRight -> StrUtil.isNotBlank(menuRight))
//                    // 过滤 METHOD 为空
